...
#define FRAME_BUFFER_1 ((layer1_pixel*)SDRAM_BASE_ADDRESS)
#define FRAME_BUFFER_2 ((layer1_pixel*)SDRAM_BASE_ADDRESS+LCD_LAYER1_BYTES)

static layer1_pixel * current_frame_buffer = FRAME_BUFFER_1;
...
{
    frame_buffer_queue = 1;
}

/** gfx expects 16 bit pixels in format RGB565 */
...
void lcd_tft_isr(void)
{
    ...
    if (frame_buffer_queue)
    {
        if (current_frame_buffer == FRAME_BUFFER_1)
        {
            current_frame_buffer = FRAME_BUFFER_2;
        }
        else
        {
            current_frame_buffer = FRAME_BUFFER_1;
        }
        /* The color frame buffer start address */
        LTDC_L1CFBAR = (uint32_t)current_frame_buffer;
        frame_buffer_queue = 0;
    }
...
{
    memset(FRAME_BUFFER_1, 0, LCD_LAYER1_BYTES);
    memset(FRAME_BUFFER_2, 0, LCD_LAYER1_BYTES);
